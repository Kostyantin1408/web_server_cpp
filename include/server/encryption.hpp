...
inline std::string make_websocket_accept(const std::string &clientKey) {
  Chocobo1::SHA1 sha;
  sha.addData(reinterpret_cast<const Chocobo1::SHA1::Byte *>(clientKey.data()), clientKey.size());
  sha.addData(reinterpret_cast<const Chocobo1::SHA1::Byte *>(WS_MAGIC.data()), WS_MAGIC.size());
  sha.finalize();
